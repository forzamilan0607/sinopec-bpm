...
bpmModel.factory('bpmService', ['$rootScope', 'baseService', 'ArrayToolService', function ($rootScope, baseService, ArrayToolService) {
     ...
     * 初始化流程数据
     */
    bpmService.initBatch = function (scope, initParam) {
        debugger;
        var param = angular.fromJson(initParam);

        var dataUrl = __ctx + "/bpm/task/getMultiTaskData";

        var defer = baseService.post(dataUrl, param);
 ...
 */
    ...
    .directive('bpmButtonsBatch', ['bpmService', 'baseService', function (bpmService, baseService) {
        return {
            restrict: "EA",
            link: function (scope, element, attrs) {
